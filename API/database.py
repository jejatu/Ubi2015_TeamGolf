...
DEFAULT_DB_PATH = "/Library/WebServer/Documents/UCF2015/API/db/logs.db"
DEFAULT_SCHEMA = "/Library/WebServer/Documents/UCF2015/API/db/db_schema.sql"
...
class ServerDatabase(object):
	...
	def updateSurvey(self, survey_id, survey_dict):
		...
		sql3 = 	("UPDATE survey_data SET "
				"age = ?, "
				"gender = ?, "
				"what_was_on_the_screen = ?, "
				"did_it_raise_positive_or_negative_emotions = ?, "
				"how_many_ads_did_you_see = ?, "
				"describe_the_ads_you_saw = ?, "
				"did_any_of_ads_gain_intrest = ?, "
				"did_ads_annoy_why = ?, "
				"ads_gained_attention = ?, "
				"found_ads_interesting = ?, "
				"might_buy = ?, "
				"disp_better_than_printed_ad = ?, "
				"disp_better_than_television_ad = ?, "
				"disp_ads_annoy_me = ?, "
				"pay_attention_to_ads = ?, "
				"often_buy_products_on_ads = ?, "
	...
	def getSurvey(self, survey_id):
		...
		OUTPUT:
			(a) returns dictionary:
				keys = survey_id, session_id, -snip-
			(b)	returns None if no session found
		'''
		with sqlite3.connect(self.db_path) as connection:
			connection.row_factory = sqlite3.Row
	...
	def getPendingSurvey(self, session_id):
		'''
		Returns the id of a survey which has not been completed.
		'''
		with sqlite3.connect(self.db_path) as connection:
			connection.row_factory = sqlite3.Row
			cursor = connection.cursor()
			...
			# Find and get the survey row:
			cursor.execute("SELECT * FROM survey_data WHERE session_id = ?", (session_id,))
			row = cursor.fetchone()
